...
const getComfortZoneText = (type: FarmTypeOption) => {
  const range = getComfortRange(type);
...
function DashboardContent() {
  ...
  // 2. Identify current context (Type & Pond)
  // We need to know the FarmType associated with the pondId if possible, 
  // but if pondId is passed, the backend handles filtering.
  // Frontend mostly needs 'type' for UI config (comfort range).
  ...
  useEffect(() => {
    ...
    if (typeParam) {
      ...
      // Fallback: if we have a pondId, we might want to find its type from profile?
      // For now, let's rely on 'type' query param being present or default SMALL.
      // Ideally FarmNavigation should pass both.
    }
  }, [searchParams]);
  ...
  return (
    <div className="min-h-screen bg-white pb-10">
      ...
      <div className="relative z-20 -mt-6 mx-4">
        {/* FarmNavigation should now link to /dashboard-farmer?pondId=... */}
        <FarmNavigation />
      </div>
            ...
            <>
              <div className="text-center mb-4">
                <p className="text-xl font-medium text-[#054DD3]">
                  {buildTemperatureDeltaText(dashboardData?.summary?.airTemperatureC, getComfortRange(farmType))}
                </p>
                <p className="text-xl font-medium text-[#054DD3]">
              ...
              <div>
                ...
                <p className="text-base text-black leading-relaxed">
                  หากอุณหภูมิน้ำอยู่ในช่วงที่เหมาะสม ({getComfortZoneText(farmType)}) ปลามักจะกินอาหารและเจริญเติบโตได้ดี ควรเฝ้าดูแนวโน้มอุณหภูมิทุกวัน
                </p>
              </div>
            </>
          )}
