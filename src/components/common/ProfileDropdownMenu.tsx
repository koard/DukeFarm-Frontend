    ...
    <div className={`flex items-center gap-3 relative ${className}`} ref={containerRef}>
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-1.5 rounded-full cursor-pointer focus:outline-none group"
          aria-haspopup="menu"
          aria-expanded={isOpen}
        >
          <div className="relative w-10 h-10">
            {/* Profile image */}
            <div className="w-10 h-10 rounded-full border-2 border-white overflow-hidden bg-gray-200 group-hover:border-white/70 transition-all">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={lineUser.pictureUrl || "/default-avatar.png"} alt="Profile" className="w-full h-full object-cover" />
            </div>
            {/* Chevron badge */}
            <div className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-white shadow-md flex items-center justify-center transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}>
